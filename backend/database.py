...
class Database:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                CREATE TABLE IF NOT EXISTS game_sessions (
                    ...
                    end_time TIMESTAMP,
                    score INTEGER,
                    tokens_collected TEXT,
                    FOREIGN KEY (player_id) REFERENCES players (player_id)
                )
            """)
            conn.commit()

    def save_player(self, player_id: str, name: str, skills: List[str]) -> None:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "INSERT INTO players (player_id, name, skills) VALUES (?, ?, ?)",
    ...
    def get_player_stats(self, player_id: str) -> Dict:
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor.execute("""
                ...
                WHERE player_id = ?
            """, (player_id,))
            avg_score, max_score, games_played = cursor.fetchone()
            return {
    ...
    def save_game_session(self, player_id: str, score: int, tokens: List[str]) -> None:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            end_time = datetime.now()
            cursor.execute("""
                INSERT INTO game_sessions 
                (player_id, start_time, end_time, score, tokens_collected)
                VALUES (?, ?, ?, ?, ?)
            """, (player_id, end_time, end_time, score, json.dumps(tokens)))
            conn.commit()
    ...
    def get_leaderboard(self, limit: int = 10) -> List[Dict]:
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.cursor()
            cursor.execute("""
                SELECT p.name, MAX(g.score) as max_score
                FROM players p
                JOIN game_sessions g ON p.player_id = g.player_id
